...
export default function Login() {
  ...
  return (
    <main className="flex min-h-screen bg-bg">
      {/* Left Panel */}
      <section className="hidden lg:flex flex-col justify-between w-[50%] bg-surface/30 p-12 relative overflow-hidden border-r border-border/50">
        ...
        <div className="relative z-10 flex items-center gap-3">
          <div className="w-8 h-8 rounded bg-gradient-primary flex items-center justify-center shadow-glow">
            <span className="material-symbols-outlined text-white text-xl italic uppercase">account_tree</span>
          </div>
          <h1 className="font-display text-3xl font-bold tracking-tight text-white italic">Oak<span className="text-primary font-black uppercase tracking-tighter">Cred</span></h1>
        </div>
        <div className="relative z-10">
          <h2 className="text-4xl font-display font-light text-white mb-6 leading-tight italic">Secure <span className="font-bold text-primary">Verification</span> for Modern Lenders.</h2>
          <p className="text-text-muted text-lg mb-8 max-w-sm italic">Standardized authentication. Direct access. No social noise.</p>
        </div>
      </section>

      {/* Right Panel */}
      <section className="w-full lg:w-[50%] p-8 md:p-16 flex flex-col justify-center items-center">
        <div className="w-full max-w-md glass-panel p-10 rounded-3xl border border-border/50 shadow-2xl">
          <div className="mb-8">
            <h2 className="font-display text-3xl font-bold text-white mb-2 italic uppercase tracking-tighter">Welcome Back</h2>
            <div className="flex gap-4 mt-6 p-1 bg-bg rounded-xl border border-border/30">
              <button onClick={() => { setLoginMode('password'); setOtpSent(false); setErrors({}); }}
                className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all italic uppercase ${loginMode === 'password' ? 'bg-primary text-white shadow-glow' : 'text-text-muted hover:text-white'}`}>Password</button>
              <button onClick={() => { setLoginMode('otp'); setErrors({}); }}
                className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all italic uppercase ${loginMode === 'otp' ? 'bg-primary text-white shadow-glow' : 'text-text-muted hover:text-white'}`}>Login Code</button>
            </div>
          </div>
          ...
          {!otpSent ? (
            <form onSubmit={loginMode === 'password' ? handlePasswordLogin : handleSendOTP} className="space-y-6">
              ...
              {loginMode === 'password' && (
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center">
                    ...
                    <a className="text-primary text-xs font-bold hover:text-white transition-colors italic" href="#">Reset?</a>
                  </div>
                  <div className="relative">
              ...
              <button type="submit" disabled={isLoading} className="w-full bg-gradient-primary text-white font-bold py-3.5 rounded-xl hover-glow transition-all uppercase tracking-widest italic disabled:opacity-50">
                {isLoading ? 'Processing...' : loginMode === 'password' ? 'Sign In' : 'Send Login Code'}
              </button>
            </form>
          ) : (
            <form onSubmit={handleVerifyOTP} className="space-y-6">
              <div className="text-center mb-6">
                <p className="text-sm text-text-muted italic">A 6-digit code has been sent to <br/><span className="text-white font-mono">{email}</span></p>
                <button type="button" onClick={() => setOtpSent(false)} className="text-xs text-primary font-bold mt-2 hover:underline italic">Change email</button>
              </div>

              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-text-muted uppercase tracking-widest text-center italic">Enter 6-Digit Code</label>
                <input type="text" value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, '').slice(0, 6))}
                  placeholder="000000" className="w-full text-center py-4 bg-bg border border-border/50 rounded-xl text-white text-2xl font-mono tracking-[0.5em] outline-none focus:border-primary transition-all placeholder:text-text-muted/20" />
          ...
          <div className="mt-8 pt-6 border-t border-border/30 text-center">
            <p className="text-sm text-text-muted italic">Don't have an account? <Link to="/signup" className="text-primary font-bold hover:text-white transition-colors">Create one</Link></p>
          </div>
        </div>
      </section>
    </main>
