...
export default function Login() {
  ...
  const isSSOAvailable = !!import.meta.env.VITE_SUPABASE_URL;
        ...
        <div className="w-full max-w-md glass-panel p-10 rounded-3xl border border-border/50">
          ...
          <div className="mt-8 pt-6 border-t border-border/50">
            ...
            <div className="flex justify-center">
              <button type="button" onClick={() => signInWithSSO('google')} disabled={isLoading || !isSSOAvailable}
                className="w-full flex items-center justify-center gap-3 py-3 bg-surface border border-border/50 rounded-xl hover:bg-surface2 transition-all group disabled:opacity-50">
                ...
                <span className="text-sm font-bold text-white/80 group-hover:text-white uppercase tracking-widest">Continue with Google</span>
              </button>
            </div>

            <div className="pt-6 border-t border-border/30 text-center">
              <p className="text-sm text-text-muted italic">
                Don't have an account? <Link to="/signup" className="text-primary hover:text-white transition-colors">Create one</Link>
              </p>
            </div>
          </div>
        </div>
