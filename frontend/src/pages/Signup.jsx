  ...
  if (isSuccess) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-bg p-6 text-center">
        <div className="glass-panel p-12 rounded-3xl max-w-lg border border-border/50 shadow-glow">
          ...
          <p className="text-text-muted text-lg leading-relaxed mb-10 italic">
            Verification link sent to <span className="text-primary font-mono font-bold">{email}</span>. Click it to activate your lender account.
          </p>
    ...
    <main className="flex min-h-screen bg-bg">
      <section className="hidden lg:flex flex-col justify-between w-[50%] bg-surface/30 p-12 relative overflow-hidden border-r border-border/50">
        ...
        <div className="relative z-10">
          <h2 className="text-4xl font-display font-light text-white mb-6 leading-tight italic">Join the <span className="font-bold text-primary text-5xl">Elite</span> AI Lending Network.</h2>
          <p className="text-text-muted text-lg mb-8 max-w-sm italic">Automated risk assessment. Institutional grade security. Email-verified access.</p>
        </div>
      </section>
      ...
      <section className="w-full lg:w-[50%] p-8 md:p-16 flex flex-col justify-center items-center">
        <div className="w-full max-w-md glass-panel p-10 rounded-3xl border border-border/50 shadow-2xl">
          <div className="mb-8">
            <h2 className="font-display text-3xl font-bold text-white mb-2 italic uppercase tracking-tighter">Create Account</h2>
            <p className="text-text-muted italic">Secure your institutional credit workspace</p>
          </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                  placeholder="name@company.com" className="w-full pl-11 pr-4 py-3 bg-bg border border-border/50 rounded-xl text-white text-sm outline-none focus:border-primary/50 transition-all font-mono italic" />
              </div>
          ...
          <footer className="mt-8 text-center text-sm text-text-muted italic">
            Already registered? <Link to="/login" className="text-primary font-bold hover:text-white transition-colors">Sign in</Link>
          </footer>
        </div>
      </section>
    </main>
